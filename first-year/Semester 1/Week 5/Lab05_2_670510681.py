...
def count_down_to_songkran(d:int,m:int,y:int) -> int:
    ...
    def sum_month(m:int,y:int):
        ...
        leap = leap_year(y)
        if m == 1: return 31
        elif m == 2: return 31 + 28 + leap
        elif m == 3: return (2*31)+(28+leap)
        elif m == 4: return (2*31)+(1*30)+(28+leap)
        elif m == 5: return (3*31)+(1*30)+(28+leap)
        elif m == 6: return (3*31)+(2*30)+(28+leap)
        elif m == 7: return (4*31)+(2*30)+(28+leap)
        elif m == 8: return (5*31)+(2*30)+(28+leap)
        elif m == 9: return (5*31)+(3*30)+(28+leap)
        elif m == 10: return (6*31)+(3*30)+(28+leap)
        elif m == 11: return (6*31)+(4*30)+(28+leap)
        elif m == 12: return (7*31)+(4*30)+(28+leap)
    ...
    def remain_day_in_month(d:int,m:int,y:int):
        ...
        if m == 2: return 28 + leap - d
            ...
            if m == 4 or m == 6 or m == 9 or m == 11: return 30 - d
            else: return 31 - d

    leap = leap_year(y)
    dayremain = remain_day_in_month(d,m,y)
    sum_ = sum_month(m,y)
    songkran = abs(103+leap-(sum_-dayremain))  # 103 from 1/1, using abs for a jump
    
    # 1-4 normal sum , 5++ add 1 year
    # check if is less or equal to 13/4
    if m == 4 and d <= 13: return 13-d
    #jump
    elif m >= 4:
