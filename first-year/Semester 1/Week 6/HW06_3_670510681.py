...
def moving_average(list_x:list[float],win_size:int) -> list[float]:
    ...
    if len(list_x) < win_size or win_size < 0:
        return []
    if win_size == 1:
        return list_x

    #index_ = list(map(lambda x:list_x.index(x),list_x)) # [0,1,2,3,4]
    index_ = list(range(len(list_x)))
    avg = list(map(lambda x:list_x[x:x+win_size],index_))
    #print(avg)
    del avg[-(win_size-1):] # remove unwanted list
    
    index_ = list(range(len(avg)))
    avg = list(map(lambda n: sum(avg[n])/win_size,index_))
